                            ...
                            <table id="kinerja" class="table table-hover responsive nowrap" style="width:100%;background-color: white">
                                ...
                                <thead>
                                    <tr style="background: #c3c3c3;">
                                        <th>No</th>
                                        <th>Realisasi Ekspor</th>
                                        <th><?= date('F Y', strtotime('-2 month'));?></th>
                                        <th><?= date('F Y', strtotime('-1 month'));?></th>
                                        <th><?= date('M', strtotime('01')).' - '.date('M', strtotime('-1 month')).' '.date('Y', strtotime('-1 years')) ;?></th>
                                        <th><?= date('M', strtotime('01')).' - '.date('M', strtotime('-1 month')).' '.date('Y') ;?></th>
                                        <th>d(%)*</th>
                                        <th>Peran(%)</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    ...
                                    foreach ($kinerja as $key => $value) {
                                        ...
                                        <tr>
                                            <td><?= $i++ ?></td>
                                            <td><?= $value->kategori ?></td>
                                            <td><?= number_format($value->nilai1, 2, ",", ".") ?></td>
                                            <td><?= number_format($value->nilai2, 2, ",", ".") ?></td>
                                            <td><?= number_format($value->nilai3, 2, ",", ".") ?></td>
                                            <td><?= number_format($value->nilai4, 2, ",", ".") ?></td>
                                            <td><?= ($value->nilai3) ? round(($value->nilai4 - $value->nilai3) / $value->nilai3 * 100, 2) : 0 ?></td>
                                          <?php if($value->nilai4==0) {?>
                                            <td>-</td>
                                            <?php }else { ?>
                                            <td><?= round($value->nilai4 / $total4 * 100, 2) ?></td>
                                            <?php }?>
                                        </tr>
                                    <?php } ?>
                                    <tr>
                    ...
                    <div class="au-card m-b-30" >
                        <div class="au-card-inner" style="overflow-x:auto">
                            ...
                            <table id="komoditi" class="table table-hover responsive nowrap" style="width:100%;background-color: white">
                                <h2 style="text-align:center">10 KOMODITI EKSPOR UTAMA JAWA TIMUR 
                                </h2>
                                    <p style="text-align:center;">Data berdasarkan BPS</p>
...
<!--                                    <div class="col-lg-4 form-group">
                                ...
                                <thead>
                                      <tr style="background: #c3c3c3;">
                                        ...
                                        <th><?= date('M', strtotime('01')).' - '.date('M', strtotime('-1 month')).' '.date('Y', strtotime('-1 years')) ;?></th>
                                        <th><?= date('M', strtotime('01')).' - '.date('M', strtotime('-1 month')).' '.date('Y') ;?></th>
                                        <th><?= date('M', strtotime('01')).' - '.date('M', strtotime('-1 month')).'<br>(M to M)'?></th>
                                        <th><?= date('Y', strtotime('-1 years')).' - '. date('Y').'<br>(Y to Y)'?></th>
                                        <th><?= 'share<br>'.date('F Y', strtotime('-1 month'));?></th>
                                        <th><?= 'share<br>'.date('M', strtotime('01')).' - '.date('M', strtotime('-1 month')).' '.date('Y') ;?></th>
                                     </tr>
                                </thead>
                                <tbody>
                                     <?php 
                                ...
                                <tr>
                                      ...
                                      <td><?=number_format($value->nilai4,2)?></td>
                                      <td>
                                          <?= ($value->nilai3) ? round(($value->nilai4 - $value->nilai3) / $value->nilai3 * 100, 2) : 0 ?>
                                      </td>
                                      <td>
                                          <?= ($value->nilai1) ? round(($value->nilai2 - $value->nilai1) / $value->nilai1 * 100, 2) : 0 ?>
                                      </td>
                                      <td>
                                          <?= ($value->nilai2) ? round(($value->nilai2 /  (($value->nilai2 - $value->nilai1) / $value->nilai1 * 100) *100),2) :0 ?>
                                      </td>
                                      <td>
                                          <?= ($value->nilai4) ? round(@($value->nilai4 / (($value->nilai4 - $value->nilai3) / $value->nilai3 * 100) *100),2) : 0?>
                                      </td>
                                </tr>
                    ...
                    <div class="au-card m-b-30" >
                        <div class="au-card-inner" style="overflow-x:auto">
                            <table id="negara" class="table table-hover responsive nowrap" style="width:100%;background-color: white">
                                <h2 style="text-align:center">10 NEGARA TUJUAN EKSPOR UTAMA JAWA TIMUR </h2>
                                <p style="text-align:center;" >Data berdasarkan BPS</p>
                                <thead>
                                ...
                                <tbody>
                                    ...
                                    foreach($negara as $key => $value){
                                            ...
                                            $total1 += ($value->nilai1) ? round($value->nilai1,2):0;
                                            $total2 += ($value->nilai2) ? round($value->nilai1,2):0;
                                            $total3 += ($value->nilai3) ? round($value->nilai1,2):0;
                                            $total4 += ($value->nilai4) ? round($value->nilai1,2):0;
                                    ...
                                    <?php } ?>
                                        ...
                                        <tr>
                                    <td>Jumlah 10 Golongan Barang</td>
                                    <td>
                                        <?= number_format($total1,2)?>
                                    </td>
                                    <td>

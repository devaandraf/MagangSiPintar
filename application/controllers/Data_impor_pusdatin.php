...
class Data_impor_pusdatin extends CI_Controller {
    
    private $filename = "dataimpor";
...
//        $data['status'] = $this->data_m->get_status();
        $this->load->view('impor/d_impor_pusdatin');       
    }
    ...
    public function upload_excel(){
        ...
        for($row = 4; $row <= $highestRow; $row++){
           ...
           $this->m_data->uploadExcelImpor($data);
           
        }
            
        redirect("Data_impor_pusdatin"); // Redirect ke halaman awal (ke controller siswa fungsi index)
    }
